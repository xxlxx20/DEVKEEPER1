...
local function kick_ban_res(extra, success, result)
  ...
  return send_large_msg(receiver, "Cannot find user by that username!")
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "زحلكه" then
         if member_id == from_id then
            send_large_msg(receiver, "لآ يہمہكہنہكہ 🗣حہضہر نہفہسہكہ♥️❗️")
   return
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "لآ يہمہكہنہكہ🖐🏽 حہظہر آلآدمہنہ آؤ آلمہديہر♥️❗️")
   return
         end
   ...
   return
        ...
        send_large_msg(receiver, 'آلعہضہؤ @'..member..' \nتہمہ بہآلفہعہل ✔️ حہظہرهہ مہنہ آلمہجہمہؤعہهہ🍾🍷')
  ban_user(member_id, chat_id)
      elseif get_cmd == 'الغاء حظر' then
        send_large_msg(receiver, 'آلعہضہؤ @'..member..' \nتہمہ آلغہآء 🖐🏽حہظہرهہ مہنہ آلمہجہمہؤعہهہ🍾🍷')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'آلعہضہؤ ['..user_id..'] \n\nتہمہ بہآلفہعہل ✔️ حہظہرهہ مہنہ آلمہجہمہؤعہهہ🍾🍷'
      elseif get_cmd == 'حظر عام' then
        send_large_msg(receiver, 'آلعہضہؤ [@'..member..'] \nتہمہ بہآلفہعہل ✔️ حہظہرهہ مہنہ كہل آلمہجہمہؤعہآتہ♥️❗️ ')
  banall_user(member_id)
      elseif get_cmd == 'الغاء العام' then
        send_large_msg(receiver, 'آلعہضہؤ [@'..member..'] \nتہمہ بہآلغہعہل آلغہآء ✖️  حہظہرهہ مہنہ كہل 🗣آلمہجہمہؤعہآتہ♥️❗️')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'ايدي' and msg.to.type == "chat" or msg.to.type == "user" then
    if msg.to.type == "user" then
      return "🔹  آيہديہ آلبہؤتہ : "..msg.to.id.. "\n\n🔹  آيہديہ حہسہآبہكہ : "..msg.from.id.. "\n\n🔹   آلمہطہؤر :  @llX8Xll "
    end
   ...
   local name = print_name:gsub("_", "")
    ...
    elseif matches[1]:lower() == 'ايدي' then
      ...
      savelog(msg.to.id, name.." ["..msg.from.id.."] used /id ")
      return "آيہديہ آلمہجہمہؤعہهہ🍾🍷" ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id
    end
  end
  ...
  msgr = get_message(msg.reply_id,ban_by_reply_admins, false)
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
           return "لآ يہمہكہنہكہ🖐🏽 حہظہر آلآدمہنہ آؤ آلمہديہر♥️❗️"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
           return "لآ يہمہكہنہكہ 🗣حہضہر نہفہسہكہ♥️❗️"
        end
        local print_name = user_print_name(msg.from):gsub("‮", "")
     local name = print_name:gsub("_", "")
  local receiver = get_receiver(msg)
        savelog(msg.to.id, name.."آلعہضہؤ ["..msg.from.id.."] \nتہمہ بہآلفہعہل ✔️ حہظہرهہ مہنہ آلمہجہمہؤعہهہ🍾🍷".. matches[2])
        ban_user(matches[2], msg.to.id)
  send_large_msg(receiver, 'آلعہضہؤ ['..matches[2]..'] \nتہمہ بہآلفہعہل ✔️ حہظہرهہ مہنہ آلمہجہمہؤعہهہ🍾🍷')
      else
  local cbres_extra = {
  chat_id = msg.to.id,
  get_cmd = 'حظر',
  ...
  if matches[1]:lower() == 'الغاء حظر' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
         ...
         local print_name = user_print_name(msg.from):gsub("‮", "")
   local name = print_name:gsub("_", "")
         savelog(msg.to.id, name.."آلعہضہؤ ["..msg.from.id.."] \nتہمہ آلغہآء 🖐🏽حہظہرهہ مہنہ آلمہجہمہؤعہهہ🍾🍷 ".. matches[2])
         return 'آلعہضہؤ '..user_id..'] \nتہمہ آلغہآء 🖐🏽حہظہرهہ مہنہ آلمہجہمہؤعہهہ🍾🍷'
      else
  local cbres_extra = {
   chat_id = msg.to.id,
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
         ...
         return 'آلعہضہؤ ['..user_id..' ] \nتہمہبہآلغہعہل آلغہآء ✖️  حہظہرهہ مہنہ كہل 🗣آلمہجہمہؤعہآتہ♥️❗️'
    else
  local cbres_extra = {
   chat_id = msg.to.id,
   get_cmd = 'الغاء العام',
