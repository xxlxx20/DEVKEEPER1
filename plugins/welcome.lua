...
local function description_rules(msg, nama)
   ...
   if data[tostring(msg.to.id)] then
      ...
      local sambutan = "🕵🏽‍♀️💋 آهلآَ وسَهلآَ كبَدَ روَحي 🍾🍷"..nama.."\n نہؤرتہ آلمہجہمہؤعہهہ كہلبہيہ🍫🌝'"..string.gsub(msg.to.print_name, "_", " ").."'\nآتہبہعہ ♻️ آلقہؤآنہيہنہ لتہجہنہبہ آلطہرد❤️‼️ \n"
      local text = sambutan.." لرؤيہة قہؤآنہيہنہ💋 آلمہجہمہؤعہه /آلقہؤآنہيہنہ 🌸🤷🏾‍♀️ "
      local text = text..""
      local text = text.."                                               "
      local text = text.."للخہرؤجہ 🌚 مہنہ آلمہجہمہؤعہهہ / آطہردنہيہ🏌🏿‍♀️🍾🍷"
      local text = text..""
      local text = text.."                    "
      local text = text.."👇🏻]"
      local text = text..""
      local text = text.."                                               "
      local text = text.."@Aaaaaaaadg☝🏿️"
      local receiver = get_receiver(msg)
...
local function run(msg, matches)
   ...
   elseif matches[1] == "chat_del_user" then
       ...
       return ' آللهہ آللهہ ؤيہآكہ يہرؤحہيہ📛❤️ '..bye_name
   end
end
